...
void ProjectX()
{
	...
	for(int i=0;i<N;i++)
	{
		for(auto u:X[i])
		{
			row[ProjColX[u.ff]] += (double)ProjSignX[u.ff]*u.ss;
		}
		Xp.push_back(row);
	}
	cout << "tracing Xp : " << N << " " << projDimx << endl; 
	for(int i=0;i<N;++i)
	{
		for(auto v:Xp[i])
			cout << v.first << ":" << v.second << " ";
...
void ProjectY()
{
	...
	for(int i=0;i<N;i++)
	{
		for(auto u:Y[i])
		{
			row[ProjColY[u.ff]] += (double)ProjSignY[u.ff]*u.ss;
		}
		Yp.push_back(row);
	}
	cout << "tracing Yp : " << N << " " << projDimy << endl; 
	for(int i=0;i<N;++i)
	{
		for(auto v:Yp[i])
			cout << v.first << ":" << v.second << " ";
...
double similarity(unordered_map<int,double> a,unordered_map<int,double> b)
{
	...
	for(auto v:a)
	{
		// cout << v.first << " " << b[v.first] << endl;
		prd += v.second*b[v.first];
	}

	// cout << "similarity : " << 1.0-double(prd)/double(sqrt(ma)*sqrt(mb)) << endl;
